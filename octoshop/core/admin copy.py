...
class PhotosLinesAdmin(nested_admin.NestedTabularInline):
    model = PhotoProduct
    def image_tag(self):
...
class ProductDetailInline(nested_admin.NestedTabularInline):
    model = ProductDetail

class ProducstInline(nested_admin.NestedStackedInline):
    model = Product
    inlines = [PhotosLinesAdmin, ProductDetailInline]
    extra = 1



class CategorysInline(nested_admin.NestedTabularInline):
    model = Category
...
class CategoryAdmin(DjangoMpttAdmin, nested_admin.NestedModelAdmin):
    ...
    search_fields = ('name',)
    exlude = ['slug']
    inlines =[CategorysInline, ProducstInline]
